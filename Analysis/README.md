...
Since RNO-G is an highly sensitive experiment, an adequate level of precision is required to analyze their data. The filtering process requires deep knowledge of the background that is being manipulated and of the tools adoperated to manupulate it. Unfortunaltely this is not always the case since the most common filtering algorithms are as simple as **defective**. [3.filtering](3.filtering) lists the several attempts of implementing these basic algorithms to adapt them for analyzing of multiple waveforms. The first algorithm examinated consists in suppressing the unwanted component in frequency space, the filtered waveform in this case is the FFT of the filtered spectrum. The second algorithm implemented filters the noise in time domain by subtracting the noise waveform from the original one. The filtered waveform is the result of this subtraction. The noise waveform has been identified with the IFFT of the noise spectrum, that is, the original spectrum where all non-noisy components have been set to zero.  
